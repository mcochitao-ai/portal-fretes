"""
WSGI config for portal_fretes project.
...
try:
    ...
    if not User.objects.filter(username='cochit0').exists():
        ...
        print("✅ Usuário cochit0 criado via WSGI!")
    else:
        # Garantir permissões
        ...
        if not user.is_staff or not user.is_superuser:
            ...
            user.is_active = True
            user.save()
            print("✅ Permissões do cochit0 atualizadas via WSGI!")
        
except Exception as e:
    print(f"⚠️ Erro ao criar usuário cochit0 via WSGI: {e}")